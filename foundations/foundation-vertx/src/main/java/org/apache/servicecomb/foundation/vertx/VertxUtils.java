...
public final class VertxUtils {
  ...
  private static Map<String, VertxProxy> vertxProxyMap = new ConcurrentHashMapEx<>();
  ...
  public static Vertx init(String name, VertxOptions vertxOptions) {
    ...
    VertxProxy vertxProxy = new VertxProxy(vertx);
    if (!StringUtils.isEmpty(name)) {
      vertxProxyMap.put(name, vertxProxy);
    }
    return (Vertx) Proxy.newProxyInstance(vertx.getClass().getClassLoader(), vertx.getClass().getInterfaces(), vertxProxy);
  }

  public static long getEventLoopContextCreatedCount(String name) {
    VertxProxy vertxProxy = vertxProxyMap.get(name);
    if (vertxProxy == null) {
      return 0;
    }
    return vertxProxy.getEventLoopContextCreatedCount();
  }
